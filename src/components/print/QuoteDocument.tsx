// Văn bản dự toán chi tiết để in.
//
// Tách khỏi route vì cùng một văn bản in được từ hai nơi: dự án và cơ hội chào giá.
// Route chỉ lo phần "ai được xem" và nạp dữ liệu; phần giấy tờ nằm ở đây.
...
export interface QuoteDocumentProps {
  ...
  /** Dòng "Công trình" — mã + tên dự án, hoặc tên công trình đang chào giá. */
  congTrinh: string;
  /** Địa điểm dự phòng khi bản dự toán không tự khai. */
  diaDiemMacDinh: string | null;
  /** Đường quay lại trang dự toán của đúng chủ sở hữu. */
  quayVe: string;
}

export function QuoteDocument({ quote, congTrinh, diaDiemMacDinh, quayVe }: QuoteDocumentProps) {
  ...
  return (
    <>
      <PrintToolbar quayVe={quayVe} nhan="Quay lại dự toán" />
      ...
      <PrintPage>
        ...
        <dl className="giu-nguyen-khoi mt-5 space-y-1 text-[11.5px]">
          ...
          <Dong nhan="Công trình" giaTri={congTrinh} />
          <Dong nhan="Địa điểm" giaTri={quote.location ?? diaDiemMacDinh} />
        ...
        <PrintSignatures
          ...
          diaDiem={quote.location ?? diaDiemMacDinh}
