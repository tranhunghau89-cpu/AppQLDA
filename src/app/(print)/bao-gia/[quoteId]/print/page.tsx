...
import { canAccessCoHoi } from "@/lib/coHoiAccess";
import { can, type Role } from "@/lib/rbac";
import { chuCuaQuote, duongDanChu } from "@/lib/quoteOwner";
 ...
 *
 * Cố ý KHÔNG mang id chủ sở hữu trên URL. Chỗ nào URL chứa hai id thì chỗ đó phải nhớ
 * đối chiếu chúng với nhau, và quên một lần là đọc được văn bản của bên khác. Ở đây chủ
 * sở hữu đọc từ chính bản ghi rồi mới hỏi quyền, nên không có gì để lệch. Thêm nữa: khi
...
export default async function ClientQuotePrintPage({
  ...
  const tenCongTrinh = quote.project
    ? `${quote.project.code} — ${quote.project.name}`
    : (quote.coHoi?.tenCongTrinh ?? null);
  const diaDiemMacDinh = quote.project?.location ?? quote.coHoi?.diaDiem ?? null;
  const quayVe = duongDanChu(chu).replace("/quote", "/client-quote");
    ...
    <>
      ...
      <PrintPage>
        ...
        <div className="mt-2 text-right text-[11px] italic">
          {dongNgayThang(quote.location ?? diaDiemMacDinh, quote.quoteDate)}
        </div>

        <dl className="giu-nguyen-khoi mt-4 space-y-1 text-[11.5px]">
          <Dong nhan="Kính gửi" giaTri={quote.recipient} dam />
          <Dong nhan="Dự án" giaTri={tenCongTrinh} dam />
          <Dong nhan="Địa điểm" giaTri={quote.location ?? diaDiemMacDinh} dam />
